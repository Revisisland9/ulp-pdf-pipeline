...
def _date_only(dt: str) -> str:
    """
    Remove time from strings like:
      '02/13/2026 08:00'
      '02/13/2026 08:00:00 AM'
    Returns just '02/13/2026'
    """
    dt = (dt or "").strip()
    if not dt:
        return ""
...
def _services_display(req: Dict[str, Any]) -> str:
    """
    Services shown on BOL (tucked under references):
      - APPT always shown
      - Liftgate shown if selected (LG / LG1 / LIFT / LIFTGATE)
    """
...
def build_shipment_confirmation_pdf(req: Dict[str, Any]) -> bytes:
    ...
    # ---------------- REFERENCES (RIGHT HALF) + SERVICES UNDER IT ----------------
    ...
    if refs:
        # No "Type / Reference Number" header row: just rows like "PO Number: 123"
        ref_rows = []
        for r in refs:
            t = s(r.get("Type"))
        ...
        # If everything got filtered out, don't render empty box
        if ref_rows:
    ...
    # Services (always show APPT; plus Liftgate if selected)
    services = _services_display(req)
    right_stack.append(Spacer(1, 0.08 * inch))
    right_stack.append(Paragraph(f"<b>Services:</b> {services}", styles["BolHeader"]))
    ...
    story.append(Paragraph(
        ...
        "whether individually determined of filed with any federal or state regulatory agency, except as "
        "specifically agreed to in writing by the listed Third Party and Carrier.",
        styles["FinePrint"],
    ))

    # A few returns before signature blocks
    story.append(Spacer(1, 0.40 * inch))

    # ---------------- SHIPPER BOX (VERBIAGE + SIGNATURE) ----------------
    shipper_box = Table(
        [[
            Paragraph(
    ...
    # ---------------- DRIVER BOX (VERBIAGE + SIGNATURE) ----------------
    driver_box = Table(
        [[
            Paragraph(
                "Carrier acknowledges receipt of packages and required four (4) placards. Carrier certifies "
                "emergency response information was made available and/or carrier has the Department of Transportation "
                "emergency response guidebook or equivalent documentation in vehicle. Property described above is "
